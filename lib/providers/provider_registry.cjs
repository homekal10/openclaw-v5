...
// ─── v5.1/v5.2: Provider Status Labels ────────────────────────────────────────
function computeProviderStatus(name) {
    const p = _registry[name];
    const h = _health[name];
    if (!p) return 'UNKNOWN';
    if (p.tier === TIERS.PAID_PLACEHOLDER || p.enabled === false) return 'DISABLED';
    // v5.2: High error count providers are never UNUSED
    if (h && h.failureCount >= 5) return 'FAILING';
    if (h && !h.healthy && h.failureCount >= 3) return 'DEGRADED';
    if (!h || (h.totalCalls === 0 && h.failureCount === 0)) return 'UNUSED';
    if (h.lastSuccessAt) {
...
// v5.2: Compute error rate percentage
function getProviderErrorRate(name) {
    const h = _health[name];
    if (!h || h.totalCalls === 0) return 0;
    return Math.round(h.failureCount / h.totalCalls * 100);
}
...
module.exports = {
    ...
    computeProviderStatus, getAllWithStatus, getProviderErrorRate,
